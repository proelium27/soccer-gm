...
- **Knockout** is format-aware (`isSwissCup`/`koRoundsOf`/`koRoundMatchdays`/`koPrizeByRound`/`koFinalRound`): Swiss = QF→SF→Final (3 rounds). Playoff + knockout ties → extra time → shootout; LP games can draw. **`simThrough` halts before the user's final** (Dashboard warns). Prizes: participation (LP) + playoff-win + per-KO-round (prize-only). Cup stats live on each tie's `BoxScore` (LP matches carry their own `boxScore`), separate from `SeasonStats`; Player Profile Cup tab.
- **Legacy cups** (pre-Swiss saves, `leaguePhase===null`): old 16-team straight bracket + `CupPlayIn` kept alive only so an in-progress old cup finishes; `migrate.ts` backfills `leaguePhase`/`playoff` to null. New cups (built each offseason via `buildCupState`) are always Swiss. Matchday layout: LP `[3,7,11,15,19,23]`, playoff `27`, KO `[31,34,37]`.
