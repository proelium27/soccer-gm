...
describe("enforceDivision2Ceiling", () => {
  it("moves an AI Division 2 player at/above the OVR threshold to a Division 1 club", () => {
    ...
    const { teams, transfers } = enforceDivision2Ceiling(
      league.teams, players, league.transfers, league.season, USER_TID,
    );
    const newTeam = teams.find((t) => t.roster.includes(star.pid))!;
    expect(newTeam.division).toBe(0);
  ...
  it("never moves a player off the user's own Division 2 roster", () => {
    ...
    const teams = league.teams.map((t) => (t.tid === USER_TID ? { ...t, division: 1 as const } : t));
    ...
    const { teams: updated } = enforceDivision2Ceiling(
      teams, players, league.transfers, league.season, USER_TID,
    );
  ...
  it("releases the receiving club's weakest player if it's already at ROSTER_CAP", () => {
    ...
    const d1TeamsByOvr = league.teams
      .filter((t) => t.division === 0 && t.tid !== USER_TID)
      .map((t) => {
  ...
  it("is deterministic for the same inputs", () => {
    ...
    const a = enforceDivision2Ceiling(league.teams, players, league.transfers, league.season, USER_TID);
    const b = enforceDivision2Ceiling(league.teams, players, league.transfers, league.season, USER_TID);
