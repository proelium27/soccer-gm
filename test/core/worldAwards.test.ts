...
describe("computeWorldAwards — cross-league competitions", () => {
  ...
  it("weights a continental championship between qualifying and a World Cup", () => {
    const stats = { caps: 6, goals: 4, assists: 1 };
    const career = (kind: "tournament" | "continental" | "qualifying") =>
    ...
    const players = [
      ...
      player({ pid: 2, tid: 2, ovr: 80, goals: 20, intl: career("continental") }),
  ...
  it("pays a continental winner's medal, and only to a nation that won one", () => {
    const line = { season: SEASON, kind: "continental" as const, caps: 5, goals: 3, assists: 1 };
    ...
    const { ballonDOr } = computeWorldAwards(
      players, SEASON, ctx({ continentalChampions: new Set(["Italy"]) }),
    );
    expect(ballonDOr.find((e) => e.pid === 1)!.intl)
      .toBeGreaterThan(ballonDOr.find((e) => e.pid === 2)!.intl);
  ...
  it("sums every campaign a player played that offseason, not just the first", () => {
    // The offseason that stages the continental championships also plays a
    ...
    const both = {
      ...
      seasons: [
        ...
        { season: SEASON, kind: "continental" as const, caps: 5, goals: 3, assists: 1 },
      ],
    };
