...
/**
 * Render harness for Frivolities' All-Time Leaders grid.
 *
 * The ranking itself is covered by `test/core/frivolities.test.ts`; what this
 * pins is the summary view — every stat gets a card, and no card runs past the
 * overview limit. The limit is the whole point of the grid: uncapped boards are
 * what it replaced.
 */
...
const { LeadersTab } = await import("../../src/ui/pages/Frivolities.js");

function render(league: LeagueStore): string {
  leagueRef.current = league;
  return renderToStaticMarkup(
    createElement(MemoryRouter, null, createElement(LeadersTab)),
  );
}

/** A retiree with a career total in every ranked stat, so no board is empty. */
