...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    items: [
      ...
      "\"Export Teams\" has been replaced by \"Export Save\", which downloads the entire save — every club, player, stat and transfer — rather than a list of club names, and which Import loads back. Together they cover backing a league up, moving it between computers, and handing it to someone else to continue. Two things to know: a save file is a snapshot rather than an editable roster file, so it isn't meant to be hand-edited and can't seed a fresh league; and it remembers which save it came from, so importing one replaces that save rather than adding a copy. To author a roster file, use the \"Copy AI Prompt to Customize\" button in the top bar of any save.",
      "Roster files now identify themselves as \"world-soccer-sim-roster\" instead of \"soccer-gm-roster\", matching the game's name. Files written with the old tag still load and always will, so nothing anyone already has stops working; only newly written files carry the new one.",
