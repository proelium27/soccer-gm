...
const entry: ChangelogEntry = {
  ...
  items: [
    ...
    "It fixed something I wasn't expecting it to. Your rating and your league's strength are the only two things in the whole calculation that mean the same in every country, and they were being drowned by those same counting stats, which is why the first version of the defender award kept going to whoever had the most defending to do in a weak league. Turning everything else up lifts them too. Over twelve test seasons, defender winners from Belgium or Turkey went from 5 in 16 down to 1 in 12, and keepers now win from the strongest leagues about as often as Ballon d'Or winners do.",
    "One thing that will look like a bug and isn't: the Goalkeeper of the Year often isn't the keeper picked in the World Team of the Year underneath it, and the same goes for the defenders. They're answering different questions. The XI picks whoever played best. The award weighs what he won as well. They land on the same man about a third of the time.",
    ...
    "The defender award still leans a bit the wrong way even after all that. Tackles and interceptions are counts, and unlike match ratings they aren't adjusted for how strong your league is, so a defender under siege every week piles up more of both than an equally good one at a club that controls its games. Weighting trophies up pushed that down from half his score to about a third rather than removing it, and centre-backs still win far more often than full-backs. Getting rid of it properly means scoring defenders against each other instead of counting what they pile up, which would change the Team of the Season in every league too, so it gets its own change rather than riding along with this one.",
