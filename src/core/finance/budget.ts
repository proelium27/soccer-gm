...
/**
 * Would this club still be solvent at its next season-start charge if it took
 * on `extraWages` of new salary on top of `currentWages`?
 *
 * This is THE affordability question for an AI acquisition, and it is not the
 * same question as "can it cover the fee". A fee is paid once out of savings;
 * a signing's *salary* is charged every season from then on, by
 * chargeSeasonStart, against a club's base income. Every AI buy path used to
 * check only the fee (the loan and transfer markets) or nothing at all (AI free
 * agency), so a club could buy comfortably inside its cash reserve in the
 * summer and then be driven negative at step 6.5 by the wage bill it had just
 * grown — which is exactly how weak-league clubs reached a deficit.
 *
 * Deliberately projects one season ahead rather than testing the live budget:
 * the live budget is a savings balance and says nothing about whether recurring
 * income covers recurring wages.
 *
 * It is a projection on the roster *as it stands at the call site*, so at the
 * free-agency call site (offseason step 4) it is only approximate, in both
 * directions: trimRosterSurplus (step 6) sheds wages afterwards, so the gate is
 * conservative and a club fine post-trim may decline a signing; but youth
 * intake (step 5) also adds a fresh contracted cohort to every AI roster before
 * the step-6.5 charge, so the gate can equally *under*-count and let a club pass
 * that is charged negative. Youth wages are small next to a senior salary, which
 * is why this is left as an approximation rather than threading a projection of
 * an intake that hasn't been generated yet. The transfer-market and loan call
 * sites run after both steps and are exact.
 */
export function staysSolvent(
  currentBudget: number,
  currentWages: number,
  extraWages: number,
  scale: number,
  ...
  return chargeSeasonStart(currentBudget, currentWages + extraWages, scale, hype) >= 0;
}
